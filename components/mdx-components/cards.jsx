...
export function Callout({ children, type = "info",title }) {
  ...
  return (
    <div className={cn(
      ...
      type === "info" && "border-blue-200 bg-blue-50 dark:border-blue-900/50 dark:bg-blue-900/20",
      type === "warning" && "border-yellow-200 bg-yellow-50 dark:border-yellow-900/50 dark:bg-yellow-900/20",
      type === "tip" && "border-green-200 bg-green-50 dark:border-green-900/50 dark:bg-green-900/20",
    )}>
      <Icon className={cn(
      ...
      <div className="prose-p:my-0 prose-a:font-medium prose-a:text-inherit prose-a:underline">
        {title && <h1>{title}</h1>}
        {children}
      </div>
    </div>
